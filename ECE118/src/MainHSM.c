...
ES_Event RunMainHSM(ES_Event ThisEvent)
{
    ...
    case InitPState: // If current state is initial Pseudo State
        if (ThisEvent.EventType == ES_INIT)// only respond to ES_Init
        {
            ...
            nextState = SearchState; //Go back to search state when ready to test on field
            makeTransition = TRUE;
    ...
    case SearchState: // in the first state, replace this with correct names
        ...
        switch (ThisEvent.EventType) {
            ...
            case GOAL_DETECTED: // 2khz detected
                nextState = EngageState;
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
                //Stop();
                break;
    ...
    case EngageState:
        ThisEvent = RunEngageSubHSM(ThisEvent);
        switch (ThisEvent.EventType) {

        case ONE_POINT_ZONE:
            printf("\r\n In the one point zone!!!\r\n");
            // When ready to program & test the firing state of the robot
            nextState = FireState;
            makeTransition = TRUE;
            ThisEvent.EventType = ES_NO_EVENT;
