...
uint8_t InitReturnToLoadSubHSM(void)
{
    ES_Event returnEvent;
    IO_PortsSetPortInputs(PORTZ, PIN4);
    CurrentState = InitPSubState;
    returnEvent = RunReturnToLoadSubHSM(INIT_EVENT);
...
ES_Event RunReturnToLoadSubHSM(ES_Event ThisEvent)
{
    ...
    case InitPSubState: // If current state is initial Psedudo State
        if (ThisEvent.EventType == ES_INIT)// only respond to ES_Init
        {
            ...
            nextState = TimeBufferState;
            makeTransition = TRUE;
            ThisEvent.EventType = ES_NO_EVENT;
        }
        break;
    case TimeBufferState:
        switch(ThisEvent.EventType){
            case ES_ENTRY:
                ES_Timer_InitTimer(ENGAGE_TIMER, 50);
            break;
            case ES_TIMEOUT:
                nextState = OrientLoad;
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
            break;
        }
    ...
    case OrientLoad:
        switch(ThisEvent.EventType){
            case ES_ENTRY:
                if (!DEFAULTTURN){
                    printf("Turning left in return to load\r\n");
                    RightMotorSpeed(75);
                    LeftMotorSpeed(-75);
                } else{
                    printf("Turning right in return to load");
                    LeftMotorSpeed(75);
                    RightMotorSpeed(-75);
                }
                ES_Timer_InitTimer(ENGAGE_TIMER, 700);
            break;
            case ES_TIMEOUT:
        ...
        switch(ThisEvent.EventType){
            ...
            case ES_TIMEOUT:
                if(ThisEvent.EventParam == ENGAGE_TIMER){
                    nextState = ExitSub;
                    makeTransition = TRUE;
                    ThisEvent.EventType = FRONTBUMPERS_TOGGLED;
                }
            break;
            default: break;
        }
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunReturnToLoadSubHSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunReturnToLoadSubHSM(ENTRY_EVENT); // <- rename to your own Run function
    }
